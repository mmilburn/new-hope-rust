...
mod tests
{
    ...
    const ODDEVEN: u16 = 0x5555;    //0b0101010101010101
    const PAIRS: u16 = 0x3333;      //0b0011001100110011
    const NIBBLES: u16 = 0x0F0F;    //0b0000111100001111
    const BYTES: u16 = 0x00FF;      //0b0000000011111111

    fn flip_unit(v: u16, pattern: u16, unit: u32) -> u16
    {
        return v.wrapping_shr(unit) & pattern | 
                (v & pattern).wrapping_shl(unit);
    }
    
    //https://en.wikipedia.org/wiki/Bit-reversal_permutation
    fn bit_reverse_permute(value: u16, bit_size: u16) -> u16
    {
        let mut v: u16 = value;
        v = flip_unit(v, ODDEVEN, 1);
        v = flip_unit(v, PAIRS, 2);
        v = flip_unit(v, NIBBLES, 4);
        v = flip_unit(v, BYTES, 8);
        v >>= 16 - bit_size;
        return v; 
    }

    fn assert_bitrev_table(table: &[u16], bit_size: u16)
    {
        for i in 0..table.len()
        {
            let index16 = i as u16;
            let expected = bit_reverse_permute(index16, bit_size);
            assert_eq!(expected, table[i], 
                       "For {}-bit table computed {} but found {} at index {}",
                       bit_size, expected, table[i], i);
        }
    }
